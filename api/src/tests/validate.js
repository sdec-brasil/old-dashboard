...
import { crypto as utils } from '../utils/crypto';
...
describe('validate', () => {
  ...
  describe('#user', () => {
    ...
    it('show throw if password does not match', () => {
      expect(() => validate.user({ password: 'password' }, 'otherpassword'))
  ...
  describe('#client', () => {
    ...
    it('show throw if client secret does not match', () => {
      expect(() => validate.client({ clientSecret: 'password' }, 'otherpassword'))
  ...
  describe('#authCode', () => {
    it('should throw with undefined code', () => {
      expect(() => validate.authCode(undefined, {
        clientID: '1',
        redirectURI: 'a',
      }, {
        id: '1',
    ...
    it('should throw with invalid client ID', () => {
      const token = utils.createToken();
      expect(() => validate.authCode(token, {
        clientID: '1',
        redirectURI: 'a',
      }, {
        id: '2',
  ...
  describe('#generateRefreshToken', () => {
    it('should generate and return a refresh token', () => validate.generateRefreshToken({ userID: '1', clientID: '1', scope: '*' })
      .then(token => utils.verifyToken(token)));
  });

  describe('#generateToken', () => {
    it('should generate and return a token', () => validate.generateToken({ userID: '1', clientID: '1', scope: '*' })
      .then(token => utils.verifyToken(token)));
  });

  describe('#generateTokens', () => {
    it('should generate and return an access and refresh token', () => validate.generateTokens({ userID: '1', clientID: '1', scope: 'offline_access' })
      .then(([accessToken, refreshToken]) => {
        utils.verifyToken(accessToken);
        utils.verifyToken(refreshToken);
      }));

    it('should generate and return an access with no refresh token when scope is defined as all', () => validate.generateTokens({ userID: '1', clientID: '1', scope: '*' })
      .then(([accessToken, refreshToken]) => {
        utils.verifyToken(accessToken);
  ...
  describe('#tokenForHttp', () => {
    it('should return 400 status', () => validate.tokenForHttp().catch(err => expect(err.status).to.eql(400)));

    it('should reject undefined token', () => validate.tokenForHttp().catch(err => expect(err.message).to.eql('invalid_token')));

    it('should reject null token', () => validate.tokenForHttp(null).catch(err => expect(err.message).to.eql('invalid_token')));

    it('should reject invalid token', () => validate.tokenForHttp('abc').catch(err => expect(err.message).to.eql('invalid_token')));
